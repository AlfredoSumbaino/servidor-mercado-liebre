...
const server = createServer((req, res) => {
  ...
  if (method === 'GET') {
    if (url === '/') {
      ...
      readFile(join(__dirname, 'views', 'home.html'), 'utf8', (err, data) => {
        if (err) {
          res.writeHead(404, { 'Content-Type': 'text/html' })
          res.end('<h1>404 Not Found</h1><p>El archivo solicitado no fue encontrado.</p>')
        }else{
      ...
      // 2. Establecer el Content-Type correcto
      // 3. Enviar el contenido al cliente
      readFile(join(__dirname, 'views', 'login.html'), 'utf8', (err, data) => {
        if (err) {
          res.writeHead(404, { 'Content-Type': 'text/html' })
          res.end('<h1>404 Not Found</h1><p>El archivo solicitado no fue encontrado.</p>')
        }else{
      ...
      // 2. Establecer el Content-Type correcto
      // 3. Enviar el contenido al cliente
      readFile(join(__dirname, 'views', 'register.html'), 'utf8', (err, data) => {
        if (err) {
          res.writeHead(404, { 'Content-Type': 'text/html' })
          res.end('<h1>404 Not Found</h1><p>El archivo solicitado no fue encontrado.</p>')
        }else{
